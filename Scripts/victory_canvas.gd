...
func _process(delta):
	...
	if is_instance_valid(Global.levelRef):
		...
		if len(Global.levelRef.winners) == 1:
			...
			$Label.text = "O jogador " + _winnerName + " venceu!";
		elif len(Global.levelRef.winners) > 1:
			# Mais de um jogador venceu:
			var winners_text = "Os jogadores"
			for i in range(len(Global.levelRef.winners)):
				if i == len(Global.levelRef.winners) - 1:
					winners_text += " e %s" % Global.levelRef.winners[i]
				elif i == 0:
					winners_text += " %s" % [Global.levelRef.winners[i]]
				else:
					winners_text += ", %s" % [Global.levelRef.winners[i]]
			$Label.text = "%s são os vencedores!" % winners_text
		else:
			# Nenhum jogador venceu.
			$Label.text = "Sem vencedores dessa vez."
	
	if !sentInfo:
		Connection.endGame($Label.text);
		sentInfo = true;
	
	$Label2.text = "Reiniciando em %s..." % floor($RestartTimer.time_left)
